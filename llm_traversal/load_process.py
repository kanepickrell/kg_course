...
def insert_docs(raw_docs: list[dict], db, replace_if_exists: bool = False, overwrite_mode: str = 'update', debug: bool = False, create_collection_on_absence: bool = True, is_edge_collection: bool = False):
    ...
    for doc in raw_docs:
        if doc is None:
            ...
            pass
        doc_type = doc['type']
        collection = get_collection(db, doc_type, create_on_absence=create_collection_on_absence, is_edge_collection=is_edge_collection)
    ...
    for key, val in artifact.items():
        if isinstance(val, list):
            if isinstance(val[0], dict):
                for sub_art in val:
                    if 'ttp_id' in sub_art:
                        # connect artifact to ttp
                        ttp_key = f"TTPArtifact/{sub_art['ttp_id']}"
                        edge_key = f"{artifact_key}_to_{ttp_key}".replace('/', ':')
                        if not any(edge.get('_key') == edge_key for edge in edges):
                            new_edges.append(create_edge('REFERENCES', edge_key, artifact_key, ttp_key, 'ttp_id', '_key'))
                        
                    if 'ttp_ids' in sub_art:
                        # connect artifact to all ttps
                        for ttp in sub_art['ttp_ids']:
                            ttp_key = f"TTPArtifact/{ttp}"
                            edge_key = f"{artifact_key}_to_{ttp_key}".replace('/', ':')
                            if not any(edge.get('_key') == edge_key for edge in edges):
                                new_edges.append(create_edge('REFERENCES', edge_key, artifact_key, ttp_key, 'ttp_ids', '_key'))
...
def handle_processes_from_file(json_file, debug=False):
...
def main():
    ...
    parser = argparse.ArgumentParser(
                    prog='load_process.py',
                    description='Loads a process (including its steps and artifact data) from a JSON file into the appropriate ArangoDB Collections.')
                    
    parser.add_argument('input_file', help='The JSON file containing the process.')
    ...
    nodes, edges = handle_processes_from_file(input_file, debug)
    ...
    edge_successes = insert_docs(edges, db, replace_if_exists=replace, create_collection_on_absence=create_collection_on_absence, debug=debug, is_edge_collection=True)
